    ...
    $('body').on('click', '.ahura-sound-player .sound-btn-play', function(e){
        ...
        if(audio && audio !== undefined){
            ...
            if(btn.hasClass('in-playlist')){
                ...
                if(soundWrap.find('.sound-player-playlist .sound-playlist-item.active').length > 0){
                    if(soundWrap.find(`.sound-player-playlist .sound-playlist-item`).last().hasClass('active')){
                    ...
                    if(soundWrap.find('.sound-player-playlist .sound-playlist-item').first().hasClass('active')){
                        soundWrap.find(`.sound-btn-prev`).addClass('btn-disabled');
                    }
                }

                soundWrap.find('.sound-player-content .sound-btn-playing').removeClass('sound-btn-play').addClass('sound-btn-pause');
            } else {
            ...
            if(ahuraGetCookie('audio_end') == 'true' || ahuraGetCookie('player_id') != playerID || ahuraGetCookie('audio_src') != audio && ahuraGetCookie('audio_hash') != btn.data('hash')){
